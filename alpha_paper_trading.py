...
/* ── COLOR TOKENS — Premium Glassmorphism Fintech ── */
:root {
  /* Surfaces — translucent glass instead of flat panels */
  --c-base:       #05070D;
  ...
  /* Dim variants */
  --c-green-dim: rgba(43,212,107,0.12);
  --c-red-dim:   rgba(245,69,79,0.12);
  --c-blue-dim:  rgba(77,127,255,0.14);
  --c-amber-dim: rgba(232,184,75,0.12);
  ...
  --r2: 6px; --r4: 12px; --r6: 16px; --r8: 16px;
...
/* ═══════════════════════════
   ...
   ═══════════════════════════ */
/* Streamlit 1.55 baseweb DOM — העטיפות שצריך לצבוע בפועל.
...
.t-kpi:hover { border-color: var(--c-border-md); }
.t-kpi-label {
  font-size: var(--fs-10);
  color: var(--c-text-2);
  text-transform: uppercase;
...
@keyframes pulseGlowGreen {
  0%, 100% { box-shadow: 0 0 0 0 rgba(43,212,107,0); }
  50%      { box-shadow: 0 0 22px 2px rgba(43,212,107,0.30); }
}
@keyframes pulseGlowRed {
  0%, 100% { box-shadow: 0 0 0 0 rgba(245,69,79,0); }
  50%      { box-shadow: 0 0 22px 2px rgba(245,69,79,0.30); }
}
.glow-green { animation: pulseGlowGreen 2.6s ease-in-out infinite; }
.glow-red   { animation: pulseGlowRed   2.6s ease-in-out infinite; }
...
def true_atr(df, period: int = 14) -> float | None:
    """
...
def render_analyst_snapshot(symbol: str, cur_price: float, ccy_s: str):
    ...
    _bd_html = _breakdown_html(symbol)
...
with st.sidebar:
    ...
    st.markdown(
        f'<div class="t-card fade-in" style="'
        f'padding:14px;margin-bottom:8px;text-align:center;">'
        f'<div style="color:var(--c-text-2);font-size:.58rem;text-transform:uppercase;margin-bottom:3px;">מניה נוכחית</div>'
        f'<div style="font-size:1.3rem;font-weight:800;color:var(--c-blue-lt);">{st.session_state.ticker}</div>'
        + (f'<div style="font-family:JetBrains Mono,monospace;font-size:.88rem;color:var(--c-text-1);">'
    ...
    st.markdown(
        '<div style="color:var(--c-text-2);font-size:.65rem;text-align:center;margin-top:8px;">'
    ...
    gc = "var(--c-green)" if gn >= 0 else "var(--c-red)"
    st.markdown(
        f'<div class="t-card fade-in" style="padding:12px;">'
        f'<div style="color:var(--c-text-2);font-size:.58rem;text-transform:uppercase;">💼 Paper Portfolio</div>'
        f'<div style="font-family:JetBrains Mono,monospace;font-size:1rem;font-weight:700;color:var(--c-text-1);margin:3px 0;">${tv:,.0f}</div>'
...
# Price + change line — Premium glass hero card
...
GROUP_TOOLTIPS = {
    ...
    "RiskShield": "RiskShield — מחשבון Black-Scholes ותנודתיות, בלי המלצות קנייה/מכירה. כולל גם סורק אסטרטגיות משולבות (ריסק ריברסל)",
...
with st.container(key="tabbody_chart"):
    ...
    def render_summary_panel(data: dict):
        ...
        st.markdown(
            f'<div class="t-card fade-in {"glow-green" if d_en=="BUY" else ("glow-red" if d_en=="SELL" else "")}" '
            f'style="padding:18px 20px;margin-bottom:10px;direction:rtl;'
            f'background:linear-gradient(160deg,{d_color}1f 0%,{d_color}08 55%,rgba(255,255,255,0.02) 100%);'
            f'border:1px solid {d_color}48;">' +
            f'<div style="display:flex;justify-content:space-between;align-items:center;">' +
            f'<div>' +
            f'<div style="font-size:1rem;font-weight:800;color:var(--c-text-1);">📋 Summary &amp; Recommendation</div>' +
            f'<div style="color:var(--c-text-2);font-size:.72rem;margin-top:2px;">סיכום ניתוח מקיף — {ticker} · {ccy_s}{lc:,.2f}</div>' +
            f'</div>' +
            f'<div style="text-align:center;">' +
            f'<div style="font-size:2rem;">{d_icon}</div>' +
            f'<div style="color:{d_color};font-size:.78rem;font-weight:700;">{d_label} / {d_en}</div>' +
            f'</div>' +
            f'</div>' +
            f'<div style="height:7px;background:rgba(255,255,255,0.08);border-radius:4px;margin-top:12px;overflow:hidden;">' +
            f'<div style="height:100%;width:{w_score}%;background:linear-gradient(90deg,{d_color}88,{d_color});border-radius:4px;transition:width 0.6s ease;"></div>' +
            f'</div>' +
            f'<div style="display:flex;justify-content:space-between;margin-top:4px;">' +
            f'<span style="color:var(--c-text-3);font-size:.6rem;">0</span>' +
            f'<span style="color:{d_color};font-size:.72rem;font-weight:600;">ציון משוקלל: {w_score}/100</span>' +
            f'<span style="color:var(--c-text-3);font-size:.6rem;">100</span>' +
            f'</div></div>',
            unsafe_allow_html=True
        )
    ...
    if not st.session_state.watchlist:
        ...
        with st.spinner("מנתח מניות..."):
            ...
            for sym_wl in st.session_state.watchlist:
                try:
                    df_wl  = load_ohlcv(sym_wl, "3mo", "1d")
            ...
            for i in range(0, len(wl_rows), 3):
                ...
                for j, r in enumerate(chunk):
                    ...
                    wcols[j].markdown(
                        ...
                        f'<span title="ציון פונדמנטלי (0-100): מבוסס על צמיחה, רווחיות, EPS, שולי רווח ומכפילים" style="cursor:help;">פונדמנטלי (F)</span><span style="font-family:JetBrains Mono,monospace;color:#388bfd;">{_f_lbl}</span></div>'
                        ...
                        f'<div style="margin-bottom:4px;">'
                        f'<div style="display:flex;justify-content:space-between;font-size:.58rem;color:#8b949e;margin-bottom:2px;">'
                        f'<span title="ציון טכני (0-100): מבוסס על מגמה, נר אחרון, ממוצעים נעים, נפח מסחר ורמות תמיכה/התנגדות" style="cursor:help;">טכני (T)</span><span style="font-family:JetBrains Mono,monospace;color:#3fb950;">{_t_lbl}</span></div>'
                        f'<div style="height:4px;background:#21262d;border-radius:2px;overflow:hidden;">'
                        f'<div style="height:100%;width:{_t_val}%;background:#3fb950;border-radius:2px;"></div></div></div>'
                        f'<div style="margin-bottom:8px;">'
                        f'<div style="display:flex;justify-content:space-between;font-size:.58rem;color:#8b949e;margin-bottom:2px;">'
                        f'<span title="ציון סיכון (0-100): מבוסס על ATR יחסי למחיר. ATR של 8% ומעלה = ציון 0 (סיכון מרבי)" style="cursor:help;">סיכון (R)</span><span style="font-family:JetBrains Mono,monospace;color:#f0883e;">{_rk_lbl}</span></div>'
                        f'<div style="height:4px;background:#21262d;border-radius:2px;overflow:hidden;">'
