...
.rr-metric { display: flex; justify-content: space-between; padding: 6px 0;
             border-bottom: 1px solid var(--c-border, rgba(255,255,255,0.08));
             font-size: 0.88rem; color: var(--c-text-1, #F4F6FB); }
.rr-metric:last-child { border-bottom: none; }
.rr-metric .k { color: var(--c-text-2, #8B93A7); }
.rr-metric .v { font-weight: 700; font-variant-numeric: tabular-nums;
...
def _metric(label: str, value: str) -> str:
    return f'<div class="rr-metric"><span class="k">{label}</span><span class="v">{value}</span></div>'
...
def _card(title: str, body_html: str) -> None:
    st.markdown(
        ...
        f'<div class="rr-title">{title}</div>{body_html}</div></div>',
